...
class Day3Tests {
    ...
    fun `Wire stores all points in a leg`() {
        ...
        assertEquals(
            Wire(
                Point(0, -3),
                mutableSetOf(Point(0, -1), Point(0, -2), Point(0, -3))
            ),
            wire
        )
    ...
    fun `Wire identifies crossover`() {
        val wire1 = Wire(points = mutableSetOf(Point(1, 0), Point(1, 1), Point(-2, 2)))
        val wire2 = Wire(points = mutableSetOf(Point(1, 0), Point(2, 0), Point(-2, 2)))

        assertEquals(setOf(Point(1, 0), Point(-2, 2)), wire1.crosses(wire2))
    }

    @Test
    fun `Calculates distance of nearest intersection`() {
        val points = setOf(Point(8, 0), Point(-2, 2))
        assertEquals(4, nearest(points))
    }
    ...
    fun `example 1`() {
        ...
        wire1.travel("R75,D30,R83,U83,L12,D49,R71,U7,L72")
        wire2.travel("U62,R66,U55,R34,D71,R55,D58,R83")

        assertEquals(159, nearest(wire1.crosses(wire2)))
    }
    ...
    fun `example 2`() {
        ...
        wire1.travel("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51")
        wire2.travel("U98,R91,D20,R16,D67,R40,U7,R15,U6,R7")

        assertEquals(135, nearest(wire1.crosses(wire2)))
    }
