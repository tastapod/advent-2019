...
class Day3Tests {
    ...
    fun `Wire stores step counts`() {
        val wire = Wire("R4,U1,L3")

        assertAll(
            { assertEquals(1, wire.steps(Point(1, 0))) },
            { assertEquals(2, wire.steps(Point(2, 0))) },
            { assertEquals(4, wire.steps(Point(4, 0))) },
            { assertEquals(7, wire.steps(Point(2, 1))) },
            { assertThrows<IllegalArgumentException> { wire.steps(Point(11, 22)) } }
        )

        wire.travel("R3")
        assertEquals(5, wire.steps(Point(4, 1))) // remembers shortest distance
    }
    ...
    fun `puzzle 2`() {
        ...
        println("Shortest intersection is at ${shortest(wire1, wire2)}")
    }
